...
- Feature 层通过协议依赖 `GeminiServiceProtocol`、`CredentialsStoreProtocol`、`AuditRepositoryProtocol`；Platform 层仅交付真实 Gemini/Keychain 实现（Mock 客户端仅供 Preview/测试注入，不面向正式用户）。
...
2. Data/Platform 服务：实现 SwiftData 存储、模板配置加载、Keychain 凭证和 Gemini 客户端（另提供 Preview 用 Mock 以便设计演示）。回滚：保留内存 Key + 文档化 API 约束。
...
5. 设置视图与 API Key 流程：实现 `⌘,` 打开设置、Key 输入校验与模型切换；无 Key 时禁用生成按钮。回滚：以占位提示阻断生成。
...
- AC4：对 Gemini 请求链路进行端到端验证（含 relay），并记录日志文件。
- AC5：痛点说明显示测试（状态驱动）、审计日志持久化与导出脚本输出，存入 `docs/Testing/scene-job-log.md`。

Rollback Points：
- Gemini 不可用：提示用户检查 API Key/网络，可选配置 relay；UI 显示离线提示。

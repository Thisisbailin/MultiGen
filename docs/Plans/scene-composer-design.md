...
**设置视图** 仍通过菜单/toolbar 打开，包含 API Key 输入（Keychain）、文本/图像模型选择、日志导出入口。

## 交互流程
1. **首启体验**：用户在 Home 查看痛点与自检状态，按提示进入设置页输入 API Key；若跳过则所有生成按钮保持禁用并提示“需先配置密钥或 relay”。
...
5. **审计与导出**：按分镜/剧本段落查看历史记录，导出日志或复制提示；设置视图可切换模型、导出日志。
...
- 提供“测试连接”“清除 Key”“查看已保存密钥”操作；Keychain 读取失败时提供修复指引。
...
| 风险 | 影响 | 对策 |
| --- | --- | --- |
| Gemini 限流/变更 | 无法生成 | 提示用户检查 API Key/网络，支持配置 relay 并提供重试策略 |
...
2. **M2**：Platform 层实现文本/图像双客户端、Keychain、模型切换与 relay 适配，完善日志输出与错误诊断。

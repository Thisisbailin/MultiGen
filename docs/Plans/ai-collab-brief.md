...
- 输出：Gemini 响应文本；若模块定义了结构化返回（如分镜表格），在智能协同中只展示摘要文本，并由对应模块处理结构化结果。
